...
## v4.1 수정
- v4 index.html에 빠졌던 `save`, `history`, `histbar` 상태 요소 추가
- 과거 데이터 프리필 시작 전에 JavaScript가 중단되던 문제 수정
- 서비스워커 캐시 버전 갱신


## v4.2 추가
- 과거 체결 120,000건을 한 번에 처리하지 않고 10,000건 단위로 나눠 저장
- 중간에 Safari/API가 끊겨도 저장된 지점부터 이어받기
- 실시간 WebSocket은 먼저 연결하고 과거 이력은 별도로 계속 축적
- 앱 복귀 시 누락 체결 보충 + 과거 이력 로딩 재개
- 서비스워커 캐시 v4.2로 갱신
